    ...
    public class SocketServerManager
    {
        ...
        static public void Init(Config.SocketServerConfig config)
        {
            ...
            foreach (Config.Server serverConfig in config.Servers)
            {
                ...
                var host = Activator.CreateInstance(typeof(SocketServer<>).MakeGenericType(
                    ...
                    serverConfig.MaxConnections, serverConfig.OneSocketPerClient) as BaseSocketServer;
        ...
        static public object GetProtocol(string protocol)
        {
            switch (protocol)
            {
                ...
                case Protocol.ProtocolNames.AsyncBinaryXMLInner: return new Protocol.AsyncBinaryXMLInnerProtocol();
                case Protocol.ProtocolNames.AsyncBinaryXMLOuter: return new Protocol.AsyncBinaryXMLOuterProtocol();
