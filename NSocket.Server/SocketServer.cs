...
namespace LW.NSocket.Server
{
    ...
    public class SocketServer<TCommandInfo> : BaseSocketServer where TCommandInfo : class, Command.ICommandInfo
    {
        ...
        public SocketServer(ISocketService<TCommandInfo> socketService,
            ...
            int maxMessageSize,
            int maxConnections, bool oneSocketPerClient = false)
            : base(socketBufferSize, messageBufferSize)
        {
            ...
            OneSocketPerClient = oneSocketPerClient;
        }
        #endregion

        public bool OneSocketPerClient { get; private set; }
        ...
        {
            if (OneSocketPerClient)
            {
                var kv = _listConnections.Get(connection.RemoteEndPoint);
                if (kv.Key > 0)
                    kv.Value.BeginDisconnect(new Exception("per client only one socket"));
            }
        ...
        {
            foreach (var child in this._listListener)
            {
                child.Start();
                LW.NSocket.SocketBase.Log.Trace.Info(child.Name + " 开始监听以下端口: " + ((IPEndPoint)child.EndPoint).Port);
            }
        }
        /// <summary>
        /// stop
