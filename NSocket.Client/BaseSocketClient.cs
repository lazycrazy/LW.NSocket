...
namespace LW.NSocket.Client
{
    ...
    public abstract class BaseSocketClient<TResponse> : SocketBase.BaseHost where TResponse : class, Response.IResponse
    {
        ...
        protected virtual void HandleUnknowResponse(TResponse response)
        {
        }
        /// <summary>
        /// OnResponse
        ...
        protected override void OnSendCallback(IConnection connection, SendCallbackEventArgs e)
        {
            ...
            if (this._requestCollection.Remove(request.UID ?? request.SeqID.ToString()) == null) return;
        ...
        public virtual void MessageReceived(TResponse response)
        {
            if (response != null)
            {
                var request = this._requestCollection.Remove(response.UID ?? response.SeqID.ToString());
                if (request == null)
                    ThreadPool.QueueUserWorkItem(_ =>
                    {
                        try { this.HandleUnknowResponse(response); }
                        catch (Exception ex) { SocketBase.Log.Trace.Error(ex.Message, ex); }
                    });
                else
                    ThreadPool.QueueUserWorkItem(_ =>
                    ...
                    ThreadPool.QueueUserWorkItem(c =>
                        {
                            ...
                            {
                                SocketBase.Log.Trace.Error(ex.Message, ex3);
                            }
                        });
                }
            }
        ...
        private class RequestCollection
        {
            ...
            private readonly int _timeout;
            private readonly Timer _timer = null;
            private readonly ConcurrentDictionary<string, Request<TResponse>> _dic = new ConcurrentDictionary<string, Request<TResponse>>();
            #endregion
            ...
            {
                this._dic.TryAdd(request.UID ?? request.SeqID.ToString(), request);
            }
            /// <summary>
            /// remove
            /// </summary>
            ...
            /// <returns></returns>
            public Request<TResponse> Remove(string uid)
            {
                Request<TResponse> removed;
                this._dic.TryRemove(uid, out removed);
                return removed;
            }
            /// <summary>
            /// clear
